    ...
    while (true) {
      ...
      switch (methodName) {
        ...
        case _methodReferences:
          ...
          // A bare class name used as a value (e.g. `references(Foo, #id)`)
          // is parsed as an Identifier in older analyzer versions, but as a
          // TypeLiteral (wrapping a NamedType) in newer ones. Support both.
          Element? staticElement;
          String? firstDisplayName;
          if (first is Identifier) {
            staticElement = first.element;
            firstDisplayName = first.name;
          } else if (first is TypeLiteral) {
            staticElement = first.type.element;
            firstDisplayName = first.type.name.lexeme;
          }
          ...
          if (staticElement is! ClassElement) {
            _resolver.reportError(DriftAnalysisError.inDartAst(
              element,
              first,
              '`$firstDisplayName` is not a class!',
            ));
            break;
          }
